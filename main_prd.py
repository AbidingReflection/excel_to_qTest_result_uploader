...
def load_config():
    config_path = r"configs\PRD.yaml"
...
def load_data_from_excel(CONFIG):
    ...
    def _validate_each_row(df):
        ...
        for idx, row in df.iterrows():
            _check_pdf_path(CONFIG, row, df, idx)
            _check_test_case_pid(row, df, idx, pid_pattern)
            _parse_test_result(row, df, idx)
            _check_unapproved_version(row, df, idx)


    def _check_pdf_path(CONFIG, row, df, idx):
        logger = CONFIG['logger']
        case_pid = row['test_case_pid']
        raw_path = row.get("pdf_file_path")
        logger.info(f"For Case '{case_pid}' confirming PDF is valid: '{raw_path}'")
        
        if not raw_path:
            logger.warning(f"Missing PDF path for Case '{case_pid}'")
        ...
        if not path.exists():
            logger.debug(f"Trying alternative path format for Case '{case_pid}'")
            alt_path = Path(clean_path_str.replace("/", "\\"))
            
            if not alt_path.exists():
                logger.error(f"PDF not found for Case '{case_pid}': {raw_path}")
                df.at[idx, "Upload Status"] = f"PDF not found: {raw_path}"
            else:
                logger.info(f"PDF found using alternative path for Case '{case_pid}': {alt_path}")
        else:
            logger.info(f"PDF found for Case '{case_pid}': {path}")
